...
_(Optional)_ Path or URL to the worker entry file (`run-ssg-worker.js`); workers are spawned from it.
...
Pick&lt;[SsgGenerateOptions](#ssginternaloptions), 'render' \| 'qwikRouterConfig' \| 'manifest'&gt;

</td><td>

Must include `render` and `qwikRouterConfig` (imported by the worker entry), plus the client `manifest` so generated loader data files match the build hash baked into the HTML. The remaining serializable options come from `workerData`.
